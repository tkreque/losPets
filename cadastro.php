...
$usuario	= 1;	//atribuição do campo "email" vindo do formulário para variavel
...
if (!empty($foto["name"])) {      
    ...
    $string_sql = "INSERT INTO `pets`.`animal` (`ani_id`, `ani_nome`, `ani_raca`, `ani_porte`, `ani_situacao`, `ani_usuario`, `ani_latitude`, `ani_longitude`, `ani_imgpath`, `ani_data`) VALUES (NULL, '$nome', '$raca', '$porte', '$situacao', '$usuario', '$latitude', '$longitude', NULL, NULL)";
    mysql_query($string_sql,$conexao);
    if(mysql_affected_rows() == 1){ //verifica se foi afetada alguma linha, nesse caso inserida alguma linha
        echo "<p>Cadastro feito com sucesso</p>";
        header("Location: http://localhost/losPets/index.html");
